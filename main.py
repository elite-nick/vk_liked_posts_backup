    ...
    try:
        ...
        for line in txt.splitlines():
            if "BANDWIDTH" in line:
                ...
                kbps = bw // 1000
                codec = (
                    line.split('CODECS="')[1].split('"')[0]
                    if "CODECS" in line else "unknown"
                )
...
def download_attachments(post):
    ...
    for att in post.get("attachments", []):
        ...
        if att["type"] == "photo":
            ...
            name = safe_filename(f"post{post_id}_photo_{att['photo']['id']}.jpg")
            download_file(url, os.path.join(DOWNLOAD_DIR, name))

        elif att["type"] == "doc":
            doc = att["doc"]
            name = safe_filename(f"post{post_id}_{doc['title']}")
            download_file(doc["url"], os.path.join(DOWNLOAD_DIR, name))
            ...
            name = safe_filename(f"post{post_id}_video_{att['video']['id']}.mp4")
            download_file(url, os.path.join(DOWNLOAD_DIR, name))
            ...
            album = a.get("album")
            cover = None
            if album and album.get("thumb"):
                cover = album["thumb"].get("photo_600") or album["thumb"].get("photo_300")
...
def process():
    ...
    while True:
        ...
        for post in r["items"]:
            ...
            liked = api("likes.isLiked", {
                "type": "post",
                "owner_id": GROUP_ID,
                "item_id": post["id"]
            })

            if liked and liked["liked"]:
                liked_count += 1
...
if __name__ == "__main__":
    if not os.path.exists(FFMPEG_PATH):
        log("ERROR: ffmpeg.exe not found with main.py")
    else:
        process()